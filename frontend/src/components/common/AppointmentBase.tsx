...
export const AppointmentBase = forwardRef<AppointmentBaseRef, AppointmentBaseProps>(({
  ...
  const timeSlots = useMemo(() => getAvailableTimeSlots(selectedDate), [selectedDate, getAvailableTimeSlots]);

  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className={`bg-neutral-50 p-6 rounded-2xl shadow-md h-fit ${isDialogMode ? "" : "max-h-125"}`}>
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <button onClick={handlePrevMonth} className="p-2 hover:bg-neutral-200 rounded-lg transition" aria-label="Mes anterior">
                <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                </svg>
              </button>
              <h3 className="text-lg font-semibold text-neutral-900 capitalize">{monthName}</h3>
              <button onClick={handleNextMonth} className="p-2 hover:bg-neutral-200 rounded-lg transition" aria-label="Mes siguiente">
                <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-7 gap-2 mb-2">
              {["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sab"].map((day) => (
                <div key={day} className="text-center text-sm font-semibold text-neutral-600">{day}</div>
              ))}
            </div>
          ...
          {selectedDate && (
            <div className="mt-6 p-4 bg-primary-50 rounded-lg border border-primary-200">
              <p className="text-sm text-neutral-700">Fecha seleccionada: <span className="font-semibold text-primary-900">{selectedDate.toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</span></p>
            </div>
          )}
        </div>

        <div className={`flex flex-col h-fit ${isDialogMode ? "" : "max-h-125"}`}>
          <div className={`mb-8 flex flex-col flex-1 ${isDialogMode ? "" : "overflow-hidden"}`}>
            <h3 className="text-lg font-semibold text-neutral-900 mb-4 shrink-0">Selecciona una hora</h3>
            {selectedDate ? (
              <div className={`space-y-3 pr-2 ${isDialogMode ? "max-h-64 overflow-y-auto" : "overflow-y-auto"}`}>
                {timeSlots.length > 0 ? (
                  timeSlots.map((slot) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span>{slot.time}</span>
                          {!slot.available && (<span className="text-xs">{isToday && minutesUntilSlot <= 30 && minutesUntilSlot > 0 ? `Ya no disponible (en ${minutesUntilSlot} min)` : "No disponible"}</span>)}
                        </div>
                      </button>
                    );
                  })
                ) : (
                  <div className="p-6 bg-neutral-100 rounded-lg text-center"><p className="text-neutral-600">Cargando horarios...</p></div>
                )}
              </div>
            ) : (
              <div className="p-6 bg-neutral-100 rounded-lg text-center"><p className="text-neutral-600">Selecciona una fecha para ver horarios disponibles</p></div>
            )}
          </div>

          <div className="bg-primary-50 p-6 rounded-2xl border border-primary-200 shrink-0 mt-4">
            <button onClick={handleOpenModal} disabled={!selectedDate || !selectedTime} className={`w-full py-3 rounded-lg font-semibold transition ${selectedDate && selectedTime ? "bg-primary-600 hover:bg-primary-700 text-white shadow-lg" : "bg-neutral-300 text-neutral-500 cursor-not-allowed"}`}>
              Agendar Cita
            </button>
          </div>
          ...
          {showUrgencia && (
            <div className="mt-4 bg-neutral-50 p-6 rounded-2xl border border-neutral-200 shrink-0">
              <button onClick={handleUrgencyClick} className="w-full py-3 rounded-lg border border-primary-600 text-primary-600 font-semibold hover:bg-primary-50 transition" dangerouslySetInnerHTML={{ __html: parseStyledText(citaUrgencia || "¿No quieres esperar? Agenda una asesoría<br />en vivo y en 10 minutos te contactamos") }} />
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex-1 px-6 mt-4">
              <div className="space-y-4 mb-6">
                <div><label className="block text-sm font-medium text-neutral-700 mb-2">Nombre *</label><input type="text" name="nombre" value={formData.nombre} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600" placeholder="Tu nombre" required /></div>
                <div><label className="block text-sm font-medium text-neutral-700 mb-2">Apellido *</label><input type="text" name="apellido" value={formData.apellido} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600" placeholder="Tu apellido" required /></div>
                <div><label className="block text-sm font-medium text-neutral-700 mb-2">Teléfono *</label><input type="tel" name="telefono" value={formData.telefono} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600" placeholder="+593 9 8822 0600" required /></div>
                <div><label className="block text-sm font-medium text-neutral-700 mb-2">Correo *</label><input type="email" name="correo" value={formData.correo} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600" placeholder="tu@email.com" required /></div>
                <div className="flex items-center">
                  <input type="checkbox" id="promociones" name="promociones" checked={formData.promociones} onChange={handleFormChange} className="w-4 h-4 rounded border-neutral-300 text-primary-600 focus:ring-primary-600" />
                  <label htmlFor="promociones" className="ml-3 text-sm text-neutral-700">Deseo recibir promociones y descuentos en mi correo</label>
                </div>
              </div>
            </div>
