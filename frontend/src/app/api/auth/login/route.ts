...
export async function POST(request: Request) {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { email },
      select: { id: true, primerNombre: true, apellido: true, email: true, password: true, rol: true, telefono: true, cedula: true, fotoPerfil: true, tokenVersion: true, loginAttempts: true, lockedUntil: true, emailVerificado: true },
    });
