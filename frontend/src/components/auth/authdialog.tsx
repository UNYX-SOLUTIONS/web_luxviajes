...
export function AuthDialog({ onClose }: AuthDialogProps) {
  ...
  // Password strength (solo para registro)
  ...
  const getPasswordStrengthScore = () => {
    ...
    if (score <= 2) return { text: "Débil", color: "text-red-500", bg: "bg-red-500" };
    if (score <= 3) return { text: "Media", color: "text-yellow-500", bg: "bg-yellow-500" };
    if (score <= 4) return { text: "Buena", color: "text-blue-500", bg: "bg-blue-500" };
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-neutral-200 sticky top-0 bg-white rounded-t-3xl z-10">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-primary-100 rounded-xl flex items-center justify-center">
                <span className="text-xl font-bold text-primary-700">L</span>
              </div>
              <div>
                ...
                <p className="text-xs text-neutral-500">
                  {mode === "login" ? "Bienvenido de vuelta" : "Únete a Lux Viajes"}
                </p>
              </div>
            </div>
            <button
                ...
                {mode === "register" && formData.password && (
                  <div className="mt-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs font-semibold ${getPasswordStrengthScore().color}`}>
                        {getPasswordStrengthScore().text}
                      </span>
                    </div>
                    <div className="w-full h-1.5 bg-neutral-200 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-300 ${getPasswordStrengthScore().bg}`}
                        style={{
